...
Второе задание: тот же пайплайн и та же база знаний, но показан инженерный цикл —
зафиксировать исходное качество, найти проблемы, внести обоснованные изменения,
повторить прогон на тех же вопросах и сравнить.

### Конфигурации

| Файл | Роль |
|---|---|
| `config/rag_baseline.yaml` | замороженный снимок baseline-настроек, **не редактировать** |
| `config/rag.yaml` | рабочая конфигурация, сюда внесены улучшения |
| `config/chunking.yaml` | baseline `chunk_overlap: 120`, улучшение — `250` |
...
context_format:  пронумерованные фрагменты с источником, версией, файлом и score
prompt:          5 правил (только по контексту, отказ при отсутствии данных,
                 запрет выдумывать числа, краткость, ссылки на фрагменты)
```

### Шаг 1. Прогон baseline

Выполняется **на исходном чанкинге** (`chunk_overlap: 120`), до внесения улучшения 1:

```bash
python src/rag_eval.py --config config/rag_baseline.yaml --label baseline
```

Результат — `results/baseline/qa_report.md` и `qa_results.json`. Оценки проставляются вручную.
Если каталог уже существует, скрипт спросит подтверждение: отчёт с проставленными оценками
терять нельзя.

### Шаг 2. Выявленные проблемы

| # | Проблема | На чём видно |
|---|---|---|
| 1 | Чанк обрывает смысловой фрагмент: заголовок кейса отделён от его числовых показателей | q2 — retrieval корректен (score 0.875, нужный документ первым), но заголовок «8.1. Горно-металлургическая компания» остался в предыдущем чанке, а цифры эффектов попали в следующий |
| 2 | Промпт провоцирует избыточный отказ: правило 2 перевешивает попытку собрать ответ из имеющихся данных | q2 — показатели присутствовали во Фрагменте 1, но модель сообщила о недостатке информации вместо того, чтобы привести их с оговоркой |
| 3 | Отсутствует similarity threshold, слабые результаты попадают в контекст | q5 — вопрос без ответа в базе, но контекст непустой; от галлюцинации удерживает только промпт |

### Шаг 3. Внесённые улучшения

**Улучшение 1 — `chunk_overlap` 120 → 250** (`config/chunking.yaml`), адресует проблему 1.
Перекрытие в 250 символов покрывает заголовок раздела и первую строку, поэтому цифры
попадают в чанк вместе с указанием кейса. Взят overlap, а не `chunk_size`, чтобы не размыть
точность на q3, который на baseline отрабатывает полностью корректно. Число чанков
вырастает с 58 до 71.

**Улучшение 2 — правила 4 и 5 промпта** (`config/rag.yaml`), адресует проблему 2.
Добавлено требование приводить относящиеся к теме числовые показатели с указанием фрагмента
и применять полный отказ только при полном отсутствии данных; отдельное правило — не сокращать
перечисления. Правка не требует переиндексации, проверяется одним прогоном.

**Улучшение 3 (опциональное) — `score_threshold`**, адресует проблему 3. Значение выставляется
по замеру: score первого фрагмента на попаданиях (q1–q4) против промаха (q5) в
`results/baseline/qa_report.md`, порог ставится между ними ближе к промаху. Слишком высокий
порог обнулит контекст на q6, где переформулировка даёт более низкий score, чем дословное
совпадение.

### Шаг 4. Прогон после улучшений

Поставить `chunk_overlap: 250` в `config/chunking.yaml`, затем:

```bash
python src/chunk_pipeline.py      # 58 -> 71 чанк
python src/embed_pipeline.py
python src/vector_pipeline.py
python src/rag_eval.py --label improved
```
...
Читает `results/baseline/qa_results.json` и `results/improved/qa_results.json`, пишет
`results/comparison.md`: таблицу изменённых параметров, сводку по вопросам (состав выдачи,
диапазон score, топовый документ, факт отказа) и поблочное «до/после» с ответами.
Столбцы «Оценка» и «Стало лучше» заполняются вручную. Если наборы вопросов в прогонах
разошлись, скрипт предупредит — сравнение на разных вопросах некорректно.
